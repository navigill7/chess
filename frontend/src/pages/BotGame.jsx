...
function BotGame() {
  ...
  }, [gameState.turn, difficulty, botThinking, playerColor]);
  ...
  const makeBotMove = async () => {
    ...
    try {
      ...
      const thinkTime = botService.getThinkingTimeForDifficulty(difficulty);

      // Add artificial delay for UX (so user sees bot "thinking")
      await new Promise(resolve => setTimeout(resolve, 300));

      const result = await botService.getBotMove(fen, thinkTime);

      if (result.success) {
        const move = result.move; // UCI format: "e2e4"
        ...
        setError('Bot failed to find a move');
      }
    } catch (err) {
      console.error('Bot move error:', err);
      setError('Bot is unavailable. Please check connection.');
    } finally {
  ...
  const handlePlayerMove = useCallback(
    (from, to) => {
      if (botThinking || gameState.status !== 'ongoing' || gameState.turn !== playerColor) {
        return;
      }
    ...
    [botThinking, gameState, playerColor, board, executeMove]
  );
  ...
  const startNewGame = (selectedDifficulty, selectedColor) => {
